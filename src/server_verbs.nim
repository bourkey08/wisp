...
macro post*(name: string, body: untyped): untyped =
    result = quote do:
        if req.reqMethod == HttpPost and curSlice(`name`):
            curPath.add(`name`)
            `body`            

macro put*(name: string, body: untyped): untyped =
    result = quote do:
        if req.reqMethod == HttpPut and curSlice(`name`):
            curPath.add(`name`)
            `body`

macro patch*(name: string, body: untyped): untyped =
    result = quote do:
        if req.reqMethod == HttpPatch and curSlice(`name`):
            curPath.add(`name`)
            `body`

macro delete*(name: string, body: untyped): untyped =
    result = quote do:
        if req.reqMethod == HttpDelete and curSlice(`name`):
            curPath.add(`name`)
            `body`

macro all*(name: string, body: untyped): untyped =
    result = quote do:
        if curSlice(`name`):
            curPath.add(`name`)
            `body`